...
// ─────────────────────────────────────────────────
// LEGACY BACK-COMPAT SHIM — DO NOT USE IN NEW CODE
// ─────────────────────────────────────────────────
// Step 1 changed the Branch shape and dropped fields (email, phoneRaw, zones,
// gmb.*, hours.weekdays/saturday/...). `src/pages/book.astro` and
// `src/pages/contact.astro` were missed in Step 1's caller scan and still
// import `{ branches }` (the old name). This shim restores the legacy shape
// just enough to keep `astro build` green until Step 3 refactors those pages.
//
// REMOVE THIS BLOCK + the `branches` export when book.astro and contact.astro
// are rewritten to consume BRANCHES directly. Tracked in the Step 3 plan.
...
/** @deprecated Use BRANCHES instead. Removed when book/contact pages are refactored in Step 3. */
export const branches = BRANCHES.map(b => ({
  ...b,
  phoneRaw: '+1' + b.phone.replace(/\D/g, ''),
  email: 'support@samedayappliance.repair',
  // For SAB locations (no public address), provide an empty PostalAddress so
  // legacy template field accesses don't crash the build. NAP rule still
  // honored: nothing real is published for non-WeHo here.
  address: b.address ?? {
    street: '',
    city: b.name,
    state: 'CA' as const,
  ...
  zones: b.citiesServed.slice(0, 8).map(slugToTitle),
  gmb: {
    status: (b.gbpStatus === 'pending' ? 'pending' : 'active') as 'active' | 'pending',
  },
  hours: { ...b.hours, ...FALLBACK_HOURS },
}));
