...
**Mobile-кадр (`hero-mobile`) — композиция центральная (2026-09-16).** Правило выше про правую
треть и пустые левые 40% относится только к широкому кадру 21:9, где карточка лежит сбоку. На
телефоне (≤768px) компонент подставляет отдельный вертикальный кадр `hero-mobile.{webp,jpg}` 9:16
(новые — 1080×1920) из той же папки. Карточка там лежит поверх всего экрана, а `object-fit: cover`
срезает кадр **с обеих сторон от центра**: блок hero на 390px — примерно 390×1100, видна только
центральная ~63% ширины кадра. Поэтому в mobile-кадре **субъект стоит по центру** и целиком внутри
центральной полосы; у краёв ничего важного. Сцена — та же, что в широком кадре страницы (тот же
техник, техника, место, свет), только повёрнута в портрет. Остальные правила (один человек, ноль
читаемых символов) — без изменений.
