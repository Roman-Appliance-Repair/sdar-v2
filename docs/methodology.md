...
## 8. Аудит = файл на диске

**Правило:** Аудит существует только как файл на диске; любой fix-промпт начинается с `cat` пути к аудиту — нет файла, нет фиксов.

**Why:** аудит, живущий только в чате одного терминала, через сессию превращается в цифры, которые никто не может проверить. Правки по таким цифрам ломают то, что не было сломано.

**Прецеденты (2026-07-16):**
- Fix-промпт ссылался на «4 direct-mismatch страницы из on-disk аудита», «April query→URL map», «May framework option B» — ни одного файла на диске не нашлось (искали по имени и по содержимому в репо, `audit-output/`, `session-log/`, вики). Фиксы не начаты, запрошены пути.
- Fix-промпт требовал откатить geo-first переупорядочивание титулов «если уже применено» — не было применено нигде: обе ветки пустые, все worktree чисты. Откатывать было нечего.
- Промпт цитировал «Smeg: позиция 1.6, ноль кликов, страница продаёт luxury» — по факту 15.9 / 1 клик, а страница уже ретро/FAB-first (FAB ×81, luxury ×1). Правка была бы порчей рабочей страницы.
- Промпт цитировал «Sub-Zero 19 pages» — в `/brands/` у Sub-Zero 6 страниц.

**How to apply:** аудит → файл (`audit-output/` или `wiki/audits/`), в промпте — путь; fix-терминал первым действием делает `cat` и сверяет цифры с реальностью (`dist`, GSC, `git`) до правок. Расхождение промпта с диском — стоп и доклад, не «примерно то же самое». См. §3 audit-first protocol.
